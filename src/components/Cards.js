...
const Cards = () => {
  ...
  return (
    ...
    data.map((e,i)=>{
        return(
      <Link to={`/detail/${e.id}`}>
      <div key={i} className='bg-sky-950 p-1 cursor-pointer hover:-translate-y-3 font-medium mt-5 transition-all duration-300'>
        ...
        <h1><span className='text-red-600'>Year :</span> {e.year}</h1>
      </div>
      </Link>
    )})
    }
